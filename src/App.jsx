...
const DERIV_APP_ID = 1089; // ⚠️ replace with your own registered app's numeric ID
...
// ─── WebSocket hook ───────────────────────────────────────────────────────────
// ─── REST helpers for the new Options API ────────────────────────────────────
const API_BASE = "https://api.derivws.com";

async function fetchAccounts(token) {
  const res = await fetch(`${API_BASE}/trading/v1/options/accounts`, {
    headers: { Authorization: `Bearer ${token}`, "Deriv-App-ID": String(DERIV_APP_ID) },
  });
...
async function fetchOtpUrl(token, accountId) {
...
function useDerivWS(token) {
  ...
  // Fetch the list of accounts (demo + real) once we have a token.
  useEffect(() => {
    if (!token) return;
    fetchAccounts(token)
      .then(list => {
        ...
        setAccounts(arr);
        // Default to the demo account if one exists, otherwise the first account.
        const def = arr.find(a => (a.account_type ?? a.type) === "demo") ?? arr[0];
        if (def) setActiveAccountId(def.id ?? def.account_id ?? def.loginid);
      })
      .catch(err => console.error("Failed to load Deriv accounts:", err));
  }, [token]);

  // Open an authenticated WebSocket for the active account via OTP — no
  // explicit `authorize` message needed; the OTP in the URL handles it.
  useEffect(() => {
    if (!token || !activeAccountId) return;
    let cancelled = false;
    let socket = null;

    fetchOtpUrl(token, activeAccountId).then(url => {
      if (cancelled) return;
      socket = new WebSocket(url);
      ws.current = socket;
      socket.onopen = () => {
        setConnected(true);
        const acc = accounts.find(a => (a.id ?? a.account_id ?? a.loginid) === activeAccountId);
      ...
      console.error("Failed to open authenticated WebSocket:", err);
      setConnected(false);
    });

    return () => { cancelled = true; setConnected(false); socket?.close(); };
  }, [token, activeAccountId, accounts]);

  const switchAccount = useCallback((id) => { setBalance(null); setActiveAccountId(id); }, []);

  return { connected, account, balance, accounts, activeAccountId, switchAccount, send, on, off };
}
...
export default function App() {
  ...
  const { connected, account, balance, accounts, activeAccountId, switchAccount, send, on, off } = useDerivWS(token);
  ...
  const buyErrorCountRef = useRef(0);
    ...
    on("contracts", d => {
      if (d.msg_type === "buy") {
        if (d.error) {
          ...
          buyErrorCountRef.current += 1;
          if (buyErrorCountRef.current >= 3) {
            // Same error repeating — don't hammer the API. Stop and surface it.
            setRunning(false);
            addLog("🛑 Stopped after repeated buy errors. Check connection/login.", "danger");
            setStatus("idle");
          } else {
            // Brief backoff before the trade loop is allowed to retry.
            setTimeout(() => setStatus("idle"), 2000);
          }
        } else {
          buyErrorCountRef.current = 0;
    ...
    <div style={{ minHeight:"100vh", background:"#f0f0f8", fontFamily:"'Inter',system-ui,sans-serif", color:"#1a1a2e", fontSize:14 }}>
      ...
      <div style={{ background:"#fff", borderBottom:"1px solid #e8eaf0", padding:"0 24px", height:54, display:"flex", alignItems:"center", gap:14, position:"sticky", top:0, zIndex:100, boxShadow:"0 1px 4px rgba(0,0,0,.05)" }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:12 }}>
          {accounts.length > 1 && (
            <div style={{ display:"flex", background:"#f4f5f9", borderRadius:10, padding:3, gap:2 }}>
              {accounts.map(a => {
                const id = a.id ?? a.account_id ?? a.loginid;
                const isReal = (a.account_type ?? a.type) !== "demo";
                const isActive = id === activeAccountId;
                return (
                  <button key={id} onClick={() => switchAccount(id)} style={{
                    padding:"6px 12px", borderRadius:8, border:"none", cursor:"pointer", fontFamily:"inherit",
                    fontSize:12, fontWeight:isActive?700:500,
                    background:isActive?"#fff":"transparent",
                    color:isActive?(isReal?"#dc2626":"#6c63ff"):"#9ca3af",
          ...
          <div style={{ display:"flex", alignItems:"center", gap:6, background:connected?"#f0fdf4":"#fef2f2", border:`1px solid ${connected?"#bbf7d0":"#fecaca"}`, borderRadius:20, padding:"4px 12px", fontSize:12, color:connected?"#16a34a":"#dc2626" }}>
            ...
            {connected?(account?.loginid ?? account?.login_id ?? "Connected"):"Connecting…"}
          </div>
      ...
      {activeTab==="profile" && (
        <div style={{ padding:24, maxWidth:500, margin:"0 auto" }}>
          <div style={{ background:"#fff", borderRadius:16, padding:24, border:"1px solid #ebebf0" }}>
            ...
            {[["Login ID",account?.loginid ?? account?.login_id ?? account?.id],["Account Type",account?.account_type ?? account?.type],["Currency",balance?.currency ?? account?.currency],["Balance",balance?`$${balance.balance.toFixed(2)}`:null],["Email",account?.email]].map(([l,v])=>(
